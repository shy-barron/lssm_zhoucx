...
<!-- 输配电 -->
  <div class="distribution">
    <base-layout>
    <!-- 列表头 -->
      <header-title :title="'输配电集团订单概览'" slot="my-header"></header-title>
      <!-- 左侧列表 -->
...
export default {
      ...
      try {
        return axios.post(group.getNuclTranStation, JSON.stringify(this.param), {
          ...
          if (res.data.data && res.data.msg === 'success' && res.status === 200) {
            ...
            this.cacheData.set('distribution', JSON.stringify(res.data.data))
  ...
  created () {
    // try {
    //   if (JSON.parse(this.cacheData.get('distribution'))) {
    //     this.synData(JSON.parse(this.cacheData.get('distribution')))
    //   }
    //   this.getData()
    // } catch (error) {
    //   console.log(error)
